...
import { Form, Select, Input, Row, Col, Button, Space, InputNumber, DatePicker, message } from 'antd';
...
const StatusSelectorHorizontal: React.FC<StatusSelectorProps> = ({ value, onChange, readOnly, hasError }) => {
    const options = [
    ...
    return (
      <Space size="small" style={{ border: hasError ? '1px solid #ff4d4f' : 'none', borderRadius: 4, padding: 2 }}>
        {options.map((opt) => (
          <div
            key={opt.val}
...
const MatchForm: React.FC<MatchFormProps> = ({ initialValues, onFinish, onCancel, readOnly, form }) => {
  ...
  const handleValuesChange = (changedValues: any) => {
    ...
    if (changedKeys.some(key => errorFields.includes(key))) {
      setErrorFields(prev => prev.filter(key => !changedKeys.includes(key) || !changedValues[key]));
    }
  };
  ...
  const teamNotFoundContent = (
    <div style={{ padding: 8, textAlign: 'center' }}>
      ...
      <Button 
        ...
        size="small" 
        onClick={() => {
          navigate('/teams', { state: { action: 'add', leagueId: selectedLeagueId } });
        }}
      >
        添加球队
      </Button>
      ...
      <Row justify="center" style={{ marginBottom: 24 }}>
        <Col span={8} style={{ textAlign: 'center' }}>
            {readOnly ? (
                ...
                <Form.Item name="leagueId" validateStatus={errorFields.includes('leagueId') ? 'error' : ''}>
      ...
      <Row justify="space-between" align="middle" style={{ marginBottom: 24 }}>
        <Col>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <Form.Item name="matchTime" getValueProps={(i) => ({ value: i ? dayjs(i) : null })} getValueFromEvent={(e) => e ? e.toISOString() : null} style={{ marginBottom: 0 }} validateStatus={errorFields.includes('matchTime') ? 'error' : ''}>
                {readOnly ? (
      ...
      <Row gutter={48} align="top">
        ...
        <Col span={8} style={{ textAlign: 'center' }}>
          <div style={{ marginBottom: 8, color: '#888' }}>
            ...
            {readOnly ? (
               ...
               <Form.Item name="homeTeamRank" noStyle>
                 <InputNumber 
                   size="small" 
                   style={{ width: 60, marginLeft: 8, border: errorFields.includes('homeTeamRank') ? '1px solid #ff4d4f' : undefined }} 
                   min={1} 
                   readOnly={!selectedHomeTeamId}
                   onClick={() => !selectedHomeTeamId && message.warning('请先选择球队')}
                 />
               </Form.Item>
            )}
          </div>
          ...
          {readOnly ? (
            ...
            <Form.Item name="homeTeamId" validateStatus={errorFields.includes('homeTeamId') ? 'error' : ''}>
                <Select 
                  placeholder="选择主队" 
                  size="large"
          ...
          <div style={{ marginTop: 24 }}>
            ...
            <Space direction="vertical">
                <Space>
                    <Form.Item name="homeForm1" noStyle><StatusSelectorHorizontal readOnly={readOnly} hasError={errorFields.includes('homeForm1')} /></Form.Item>
                    <Form.Item name="homeForm2" noStyle><StatusSelectorHorizontal readOnly={readOnly} hasError={errorFields.includes('homeForm2')} /></Form.Item>
                    <Form.Item name="homeForm3" noStyle><StatusSelectorHorizontal readOnly={readOnly} hasError={errorFields.includes('homeForm3')} /></Form.Item>
                </Space>
                <Space>
                    <Form.Item name="homeForm4" noStyle><StatusSelectorHorizontal readOnly={readOnly} hasError={errorFields.includes('homeForm4')} /></Form.Item>
                    <Form.Item name="homeForm5" noStyle><StatusSelectorHorizontal readOnly={readOnly} hasError={errorFields.includes('homeForm5')} /></Form.Item>
                    <Form.Item name="homeForm6" noStyle><StatusSelectorHorizontal readOnly={readOnly} hasError={errorFields.includes('homeForm6')} /></Form.Item>
                </Space>
            </Space>
          </div>
        </Col>
        ...
        <Col span={8} style={{ textAlign: 'center' }}>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <Space direction="vertical">
                <Form.Item name="historyH2H1" noStyle><StatusSelectorHorizontal readOnly={readOnly} hasError={errorFields.includes('historyH2H1')} /></Form.Item>
                <div style={{ height: 8 }} />
                <Form.Item name="historyH2H2" noStyle><StatusSelectorHorizontal readOnly={readOnly} hasError={errorFields.includes('historyH2H2')} /></Form.Item>
                <div style={{ height: 8 }} />
                <Form.Item name="historyH2H3" noStyle><StatusSelectorHorizontal readOnly={readOnly} hasError={errorFields.includes('historyH2H3')} /></Form.Item>
            </Space>
          </div>
        ...
        <Col span={8} style={{ textAlign: 'center' }}>
          <div style={{ marginBottom: 8, color: '#888' }}>
            ...
            {readOnly ? (
               ...
               <Form.Item name="awayTeamRank" noStyle>
                 <InputNumber 
                   size="small" 
                   style={{ width: 60, marginLeft: 8, border: errorFields.includes('awayTeamRank') ? '1px solid #ff4d4f' : undefined }} 
                   min={1} 
                   readOnly={!selectedAwayTeamId}
                   onClick={() => !selectedAwayTeamId && message.warning('请先选择球队')}
                 />
               </Form.Item>
            )}
          </div>
          ...
          {readOnly ? (
            ...
            <Form.Item name="awayTeamId" validateStatus={errorFields.includes('awayTeamId') ? 'error' : ''}>
                <Select 
                  placeholder="选择客队" 
                  size="large"
                  onDropdownVisibleChange={handleTeamDropdownVisibleChange}
          ...
          <div style={{ marginTop: 24 }}>
            ...
            <Space direction="vertical">
                <Space>
                    <Form.Item name="awayForm1" noStyle><StatusSelectorHorizontal readOnly={readOnly} hasError={errorFields.includes('awayForm1')} /></Form.Item>
                    <Form.Item name="awayForm2" noStyle><StatusSelectorHorizontal readOnly={readOnly} hasError={errorFields.includes('awayForm2')} /></Form.Item>
                    <Form.Item name="awayForm3" noStyle><StatusSelectorHorizontal readOnly={readOnly} hasError={errorFields.includes('awayForm3')} /></Form.Item>
                </Space>
                <Space>
                    <Form.Item name="awayForm4" noStyle><StatusSelectorHorizontal readOnly={readOnly} hasError={errorFields.includes('awayForm4')} /></Form.Item>
                    <Form.Item name="awayForm5" noStyle><StatusSelectorHorizontal readOnly={readOnly} hasError={errorFields.includes('awayForm5')} /></Form.Item>
                    <Form.Item name="awayForm6" noStyle><StatusSelectorHorizontal readOnly={readOnly} hasError={errorFields.includes('awayForm6')} /></Form.Item>
                </Space>
            </Space>
          </div>
